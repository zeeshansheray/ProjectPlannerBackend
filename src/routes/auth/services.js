...
const LoginService = async (reqData) => {
    const result = await User.findOne({email: reqData.email , types : { $in: [reqData.type] }})
    if(!result)
    return {
        success : false,
...
const GetUserService = async (reqData) => {
    let result;

    if (reqData._id) {
      result = await User.find({ _id: reqData._id });
    }
  
    if (reqData.type) {
      result = await User.find({ types: { $in: [parseInt(reqData.type)] } });
    }
  
    if (result.length <= 0) {
      throw { message: Responses.LoginResponse.USER_NOT_FOUND };
    }
  
    return {
      success: true,
      message: Responses.LoginResponse.FOUND,
      data: result,
    };
  };
